...
class CumulantAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            load_lens: Whether to load the tuned lens.
            revision: Optional git revision/branch/tag to use.
            cache_mode: Cache storage mode:
        ...
        if cache_mode == "temp":
            ...
            if cache_dir is None:
                raise ValueError("cache_dir must be provided when cache_mode='custom'")
    ...
    def _load_model(self, revision: Optional[str] = None) -> Tuple[AutoModelForCausalLM, AutoTokenizer]:
        ...
        """
        # Set up kwargs for model and tokenizer loading
        model_kwargs = {
            'device_map': 'auto',
        }
        tokenizer_kwargs = {}
    ...
    def _load_lens(self) -> TunedLens:
        ...
        if self.cache_dir is not None:
            tuned_lens = TunedLens.from_model_and_pretrained(self.model, cache_dir=self.cache_dir)
        else:
